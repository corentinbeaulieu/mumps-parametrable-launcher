...
symmetry=$(echo $symmetry | cut -d '.' -f1)
...
num_threads=$(echo $num_threads | cut -d '.' -f1)
...
num_proc=$(echo $num_proc | cut -d '.' -f1)
...
icntl_13=$(echo $icntl_13 | cut -d '.' -f1)
...
ordering=$(echo $ordering | cut -d '.' -f1)

rand=$RANDOM
outputfile="tmp${rand}.out"

# Log the inputs
printf "%s %d %s %s %d %d %d %d %d %d " "$type" "$n" "$bandwidth" "$density" "$symmetry" "$num_proc" "$num_threads" "$par" "$icntl_13" "$ordering" >> launch.log
...
KMP_AFFINITY=scatter OMP_NESTED=TRUE MKL_NUM_THREADS="$num_threads" OMP_NUM_THREADS="$num_threads"\
    salloc -N 1 -n "$num_proc" -c "$num_threads" --job-name=mumps_run -p cpu_short --mem=32G --time=00:05:00\
    srun ./mumps\
    "-ag" "$type_int" "$n" "$bandwidth" "$density" "$symmetry" "$par" "$icntl_13" "$num_threads" "$ordering"\
    1> "$outputfile" 2>> err.out

OMP_NESTED=TRUE OMP_NUM_THREADS="$num_threads" \
    mpirun -np "$num_proc" --mca accelerator rocm ./mumps "-a" "0" "$n" "$bandwidth" "$density" "$symmetry" "$par" "$icntl_13" "$num_threads" "$ordering" 1> "$outputfile" 2>> err.out

exec_error=$(grep -o "** ERROR RETURN ** FROM DMUMPS INFO(1)=" "$outputfile")

# Check for an error
if [[ $exec_error != "" ]]; then
    if [[ $(echo $exec_error | grep -E -o "-7[0-9]") == "" ]]; then
...
total=$(echo "scale=4; $analysis_time" | bc)
printf "%s\n" "$total" >> launch.log
echo "$analysis_time"
